...
import React, { useContext } from "react";
import { useParams, useNavigate } from "react-router-dom";
import { StoreContext } from "../store/ContextProvider"; // Updated the context name

function SingleItem() {
  const { id } = useParams();
  const navigate = useNavigate();
  
  // Use the context to get product data
  const {
    state: { products },
  } = useContext(StoreContext); // Ensure the context name matches
